...
function UserLogin() {
  ...
  const { userToken, tokenDispatch } = useContext(TokenContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem(
        "clientName",
        JSON.stringify(result.data.client.name)
      );
      localStorage.setItem("clientId", JSON.stringify(result.data.client._id));
    } catch (error) {
        ...
        <div className="px-6 h-full text-gray-800">
          <div className="flex xl:justify-center lg:justify-between justify-center items-center flex-wrap h-full g-6">
            <div className="mt-6 grow-0 shrink-1 md:shrink-0 basis-auto xl:w-6/12 lg:w-6/12 md:w-9/12 mb-12 md:mb-0">
              <img src="userlogin2.png" className="w-4/5" alt="Sample" />
            </div>
